...
import { z } from "zod";
import { insertContactSchema, insertThemeSettingsSchema, insertSystemSettingSchema } from "@myhealthintegral/shared";
...
import { mediaPositionsPublicRouter, mediaPositionsAdminRouter } from "./modules/media-positions";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/v1/public/contact", asyncHandler(async (req, res) => {
    ...
    import('./services/zoho.service').then(({ zohoCRM }) => {
      zohoCRM.createLead(contact).then((result) => {
        if (result.success) {
          console.log(`✅ Zoho CRM lead created for ${contact.email} (ID: ${result.id})`);
        } else {
          console.warn(`⚠️ Failed to sync to Zoho CRM for ${contact.email}: ${result.error}`);
        }
      }).catch((error) => {
  ...
  app.post("/api/v1/admin/contacts/sync-zoho", authenticateToken, requirePermission("content", "update"), asyncHandler(async (req, res) => {
  ...
  app.get("/api/v1/admin/audit-logs", authenticateToken, requirePermission("users", "read"), asyncHandler(async (req, res) => {
  ...
  app.put("/api/v1/admin/settings", authenticateToken, requirePermission("users", "update"), asyncHandler(async (req, res) => {
    ...
    await logUserAction(currentUser.id, "update", "settings", setting.id, { 
  ...
  app.post("/api/v1/admin/themes", authenticateToken, requirePermission("themes", "create"), asyncHandler(async (req, res) => {
  ...
  app.put("/api/v1/admin/themes/:id", authenticateToken, requirePermission("themes", "update"), asyncHandler(async (req, res) => {
    const { id } = req.params;
    const currentUser = (req as any).user;
  ...
  app.delete("/api/v1/admin/themes/:id", authenticateToken, requirePermission("themes", "delete"), asyncHandler(async (req, res) => {
    const { id } = req.params;
    const currentUser = (req as any).user;
  ...
  app.patch("/api/v1/admin/themes/:id/activate", authenticateToken, requirePermission("themes", "update"), asyncHandler(async (req, res) => {
    const { id } = req.params;
    const currentUser = (req as any).user;

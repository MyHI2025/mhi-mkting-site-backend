...
import type { 
  Page, InsertPage,
  ContentNode, InsertContentNode,
  ContentSection, InsertContentSection,
  ContentBlock, InsertContentBlock,
  Contact, InsertContact,
  PageVersion, InsertPageVersion
} from '@myhealthintegral/shared';
import { pages, contentNodes, contentSections, contentBlocks, contacts, pageVersions } from '@myhealthintegral/shared';
import { db } from '../../db';
...
export class ContentRepositoryImpl implements IContentRepository {
  ...
  async updatePage(id: string, data: Partial<Page>): Promise<Page> {
    ...
    const currentVersionNumber = (await this.getLatestVersion(id))?.versionNumber || 0;
    await this.createPageVersion({
      pageId: id,
      versionNumber: currentVersionNumber + 1,
      title: data.title || page.title,
      description: data.description !== undefined ? data.description : page.description || undefined,
      pageType: page.pageType,
      category: data.category !== undefined ? data.category : page.category || undefined,
      metaTitle: data.metaTitle !== undefined ? data.metaTitle : page.metaTitle || undefined,
      metaDescription: data.metaDescription !== undefined ? data.metaDescription : page.metaDescription || undefined,
      featuredImage: data.featuredImage !== undefined ? data.featuredImage : page.featuredImage || undefined,
      metadata: data.metadata !== undefined ? data.metadata : page.metadata || undefined,
      isPublished: data.isPublished !== undefined ? data.isPublished : page.isPublished || false,
      changeType,
      changeSummary,
    });
  ...
  async getNodesByPageId(pageId: string): Promise<ContentNode[]> {
    return await db.select()
      .from(contentNodes)
      .where(and(
        eq(contentNodes.pageId, pageId),
        isNull(contentNodes.parentId)
      ))
  ...
  async updateNode(id: string, data: Partial<ContentNode>): Promise<ContentNode> {
    const [updatedNode] = await db.update(contentNodes)
      .set({
        ...data,
        updatedAt: new Date(),
      })
  ...
  async deleteNode(id: string): Promise<{ success: boolean; message: string }> {
    const result = await db.delete(contentNodes)
      .where(eq(contentNodes.id, id))
      .returning();
  ...
  async findSectionById(id: string): Promise<ContentSection | null> {
    const result = await db.select().from(contentSections).where(eq(contentSections.id, id)).limit(1);
    return result[0] || null;
  }
  ...
  async updateSection(id: string, data: Partial<ContentSection>): Promise<ContentSection> {
  ...
  async updateBlock(id: string, data: Partial<ContentBlock>): Promise<ContentBlock> {
  ...
  async deleteBlock(id: string): Promise<{ success: boolean; message: string }> {
  ...
  async createPageVersion(data: InsertPageVersion, createdBy?: string): Promise<PageVersion> {
    const id = randomUUID();
    const now = new Date();
  ...
  async restorePageVersion(pageId: string, versionId: string, userId?: string): Promise<Page> {
  ...
  async compareVersions(versionId1: string, versionId2: string): Promise<{
    ...
    const fields = ['title', 'description', 'category', 'metaTitle', 'metaDescription', 'featuredImage', 'isPublished'];

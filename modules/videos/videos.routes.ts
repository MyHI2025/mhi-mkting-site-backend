...
import { insertVideoContentSchema } from '@myhealthintegral/shared';
...
router.post("/", authenticateToken, requirePermission("content", "create"), async (req, res) => {
  try {
    ...
    const newVideo = await videosService.createVideo(validatedData, currentUser.id, req);
...
router.put("/:id", authenticateToken, requirePermission("content", "update"), async (req, res) => {
  try {
    ...
    const updatedVideo = await videosService.updateVideo(id, validatedData, currentUser.id, req);
